...
// ensure fluent-ffmpeg knows where ffmpeg binary is
if (ffmpegStatic) {
  ...
  return new Promise<FileInterface>((resolve, reject) => {
    try {
      ...
      ffmpeg(decryptedReadStream, {
        ...
        .on("start", (command) => {
          console.log("FFmpeg command started:", command);
        })
        .on("end", async (err) => {
          console.log("FFmpeg processing ended", err);
          if (!emitter) {
            console.log("No emitter, handling finish directly");
            await handleFinish();
          }
        })
        .on("error", async (err, sdf, stderr) => {
          console.log("thumbnail error attempting temp directory fix: ", err, stderr);
          console.log(`env.tempDirectory: ${env.tempDirectory} && env.tempVideoThumbnailLimit: ${env.tempVideoThumbnailLimit} > file.length: ${file.length}`);
